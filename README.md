...
[功能](#-核心功能) · [快速开始](#-快速开始) · [部署](#-部署) · [在线演示](#-在线演示) · [截图](#-截图) · [路线图](#-路线图) · [贡献](#-贡献) · [许可证](#-许可证)
...
- **掉线/恢复告警** — 自动检测状态切换，弹窗告警 + 历史记录 + 时间范围筛选
- **历史图表** — Chart.js 双 Y 轴折线图（在线人数 + 延迟）
- **服务器分组** — 多分组管理，每个服务器可归属一个分组
- **搜索 + 状态筛选** — 客户端即时过滤
- **自定义排序** — 通过排序弹窗手动调整服务器显示顺序
...
🚀 **[点击查看 Showcase 演示页面](https://forgetmelodyXL.github.io/mc-monitor/showcase/)**

一个纯静态的产品展示页面，包含功能介绍、界面预览和特性说明，托管在 GitHub Pages 上。

> 💡 实际运行的监控面板需要自行部署，参见下方「快速开始」。
...
```bash
curl -fsSL https://raw.githubusercontent.com/forgetmelodyXL/mc-monitor/main/install.sh | bash
```

脚本自动检测环境，优先使用 Docker 部署，没有 Docker 则用 Python 原生部署。支持 Debian / Ubuntu / CentOS / RHEL / Arch / Alpine 等主流发行版。

还提供了更新和卸载脚本：
```bash
# 更新
curl -fsSL https://raw.githubusercontent.com/forgetmelodyXL/mc-monitor/main/update.sh | bash

# 卸载
curl -fsSL https://raw.githubusercontent.com/forgetmelodyXL/mc-monitor/main/uninstall.sh | bash
```
...
`launch.bat` 会自动检测 Python 环境，缺失则自动下载便携版 Python，然后启动服务。
...
`build.bat` 使用 PyInstaller 将项目打包为单文件 EXE，输出到 `dist\mcmonitor.exe`。
...
设置 `MCMONITOR_BOOTSTRAP_PASSWORD` 环境变量，首次启动时自动创建用户名为 `admin` 的超级管理员，密码为你设置的值：

```bash
export MCMONITOR_BOOTSTRAP_PASSWORD="your_secure_password"
```

> 💡 密码长度至少 8 位。仅当数据库中没有任何用户时才会触发创建。

### 方式二：自动生成临时管理员

首次启动时，如果数据库为空且**未设置** `MCMONITOR_BOOTSTRAP_PASSWORD`，系统会**自动创建临时管理员账号**，并在控制台输出随机密码：
...
> ```bash
> # Windows EXE
> mcmonitor.exe monitor reset-password
>
> # Python 源码
> python cli.py monitor reset-password
>
> # 指定新密码（至少8位）
> python cli.py monitor reset-password mynewpassword
> ```
>
> 不指定密码时会自动生成随机密码并打印到控制台。
...
数据持久化在 `./data` 目录下（包括数据库和日志）。

### 环境变量配置

完整可配置项见 [.env.example](.env.example)：
...
| `MCMONITOR_DATABASE` | `mcmonitor.db` | 数据库文件路径 |
...
main.py           # 启动入口（含 sys._MEIPASS 处理 / CLI 命令分发）
cli.py            # 命令行工具（monitor 前缀指令）
db.py             # 数据库初始化与迁移
templates/        # HTML 模板
...
部署者可通过 `monitor` 前缀的命令行指令进行后台管理操作。

### 使用方式

```bash
# Windows EXE
mcmonitor.exe monitor <命令> [参数]

# Python 源码
python cli.py monitor <命令> [参数]
```
...
| 命令 | 说明 |
|------|------|
| `monitor reset-password [密码]` | 重置 admin 密码，不指定则随机生成 |
| `monitor create-user <用户名> [密码]` | 创建普通用户 |
| `monitor list-users` | 列出所有用户 |
| `monitor version` | 查看版本号 |
| `monitor help` | 显示帮助信息 |
...
# 重置 admin 密码（指定密码，至少8位）
python cli.py monitor reset-password mysecurepass

# 创建普通用户
python cli.py monitor create-user testuser testpass123

# 列出所有用户
python cli.py monitor list-users
```

### 交互式模式

服务运行时，可直接在控制台窗口输入 `monitor` 命令进行管理，无需重启程序：

```
  💡 交互式命令行已启用，可直接输入 monitor 命令进行管理
  输入 help 查看可用命令

monitor reset-password
monitor list-users
monitor help
clear
```

输入 `help` 查看帮助，输入 `clear` / `cls` 清屏。关闭窗口或按 `Ctrl+C` 停止服务。
