...
1. 下载/克隆本仓库
2. 双击 [`launch.bat`](launch.bat)
3. 首次启动会自动：
   - 下载便携版 Python（约 10 MB，免安装）
   - 安装 Flask / requests / APScheduler
   - 启动 Web 服务器
4. 浏览器自动打开 `http://127.0.0.1:5000`

> 💡 之后每次使用只需要双击 `launch.bat` 即可。
...
# 在开发机上
build.bat
# 输出：dist\mcmonitor.exe (~30 MB，单文件，自包含 Python)
```

将 `mcmonitor.exe` 复制到任何 Windows 10/11 机器，**无需安装 Python**，双击即可运行。
...
**Q: 忘了管理员密码？**
> 删掉 `mcmonitor.db` 后重启，会自动重新生成 `admin/admin`。
